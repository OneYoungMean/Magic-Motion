...
namespace BioIK2
{
    ...
    public class BioMotion : MonoBehaviour
    {
        ...
        public float3 constraintValue = 1;//OYM:Ô¼Êø?Ô¼ÊøÉ¶?
        [SerializeField]
        ...
        {
            float3 constraintMul = (1- constraintValue) * float.MinValue + 1;//OYM£ºfalse =minvalue ,true =1
        ...
        {
            float3 constraintMul = (1 - constraintValue) * float.MaxValue + 1;//OYM£ºfalse =maxvalue ,true =1
            float3 result = 0;
            if  (joint.jointType == BioJointType.Rotational)
            {
