...
namespace BIOIK
{
    ...
    */
    ...
    partial class BFGS_F
    {
        ...
        // 
        private static void active(int n, float[] l, int _l_offset, float[] u, int _u_offset, int[] nbd,
            int _nbd_offset, float[] x, int _x_offset, int[] iwhere, int _iwhere_offset, int iprint,
            ...
            // 
            {
                for (i = 1; i <= n; i++)
                {
                    ...
                    else
                    {
                        cnstnd = true;
                        if (((nbd[(i - (1)) + _nbd_offset] == 2) &&
                            ((u[(i - (1)) + _u_offset] - l[(i - (1)) + _l_offset]) <= 0)))
                        {
        ...
        private static void dtrsl(float[] t, int _t_offset, int ldt, int n,
            float[] b, int _b_offset, int job, ref int info)
        {
            float temp = 0f;
            int Case = 0;
            int j = 0;
...

            {
                ...
                {
                    // ......exit
                    if ((t[(info - (1)) + (info - (1)) * (ldt) + _t_offset] == 0))
                    {
                        return;
                    }
                }
        ...
        // c       in the compact L-BFGS_F formula of B and a 2m vector v;  
        ...
        // c
        // c     sy is a float precision array of dimension m x m.
        // c       On entry sy specifies the matrix S'Y.
        // c       On exit sy is unchanged.
        // c
        // c     wt is a float precision array of dimension m x m.
        ...
        // c         stored in the compact L-BFGS_F formula.
        // c       On exit col is unchanged.
        // c
        // c     v is a float precision array of dimension 2col.
        // c       On entry v specifies vector v.
        // c       On exit v is unchanged.
        // c
        // c     p is a float precision array of dimension 2col.
        ...
        // 
        // 
        private static void bmv(int m, float[] sy, int _sy_offset, float[] wt, int _wt_offset,
            int col, float[] v, int _v_offset, float[] p, int _p_offset, ref int info)
        {
        ...
        // c       BFGS_F matrix B defined in terms of matrices WY, WS, WT, and
        ...
        // c
        // c     x is a float precision array of dimension n.
        // c       On entry x is the starting point for the GCP computation.
        // c       On exit x is unchanged.
        // c
        // c     l is a float precision array of dimension n.
        // c       On entry l is the lower bound of x.
        // c       On exit l is unchanged.
        // c
        // c     u is a float precision array of dimension n.
        ...
        // c
        // c     g is a float precision array of dimension n.
        ...
        // c
        // c     t is a float precision working array of dimension n. 
        // c       t will be used to store the break points.
        // c
        // c     d is a float precision array of dimension n used to store
        // c       the Cauchy direction P(x-tg)-x.
        // c
        // c     xcp is a float precision array of dimension n used to return the
        ...
        // c       On exit m is unchanged.
        // c
        // c     ws, wy, sy, and wt are float precision arrays.
        // c       On entry they store information that defines the
        // c                             limited memory BFGS_F matrix:
        ...
        // c
        // c     theta is a float precision variable.
        ...
        // c       On exit col is unchanged.
        // c
        // c     p is a float precision working array of dimension 2m.
        // c       p will be used to store the vector p = W^(T)d.
        // c
        // c     c is a float precision working array of dimension 2m.
        // c       c will be used to store the vector c = W^(T)(xcp-x).
        // c
        // c     wbp is a float precision working array of dimension 2m.
        // c       wbp will be used to store the row of W corresponding
        // c         to a breakpoint.
        // c
        // c     v is a float precision working array of dimension 2m.
        // c
        // c     nseg is an integer variable.
        // c       On exit nseg records the number of quadratic segments explored
        // c         in searching for the GCP.
        // c
        // c     sg and yg are float precision arrays of dimension m.
        ...
        // c
        // c     sbgnrm is a float precision variable.
        ...
        // c     Subprograms called:
        // c 
        // c       L-BFGS_F-B Library ... hpsolb, bmv.
        // c
        ...
        // c
        // c       [2] C. Zhu, R.H. Byrd, P. Lu, J. Nocedal, ``L-BFGS_F-B: FORTRAN
        ...
        // c        ftp to eecs.nwu.edu in the directory pub/lBFGS_F/lBFGS_F_bcm.)
        // c
        // c                           *  *  *
        // c
        // c     NEOS, November 1994. (Latest revision June 1996.)
        ...
        // 
        private static void cauchy(int n, float[] x, int _x_offset, float[] l, int _l_offset,
            float[] u, int _u_offset, int[] nbd, int _nbd_offset, float[] g, int _g_offset,
            int[] iorder, int _iorder_offset, int[] iwhere, int _iwhere_offset, float[] t, int _t_offset,
            float[] d, int _d_offset, float[] xcp, int _xcp_offset, int m,
            float[] wy, int _wy_offset, float[] ws, int _ws_offset, float[] sy, int _sy_offset,
            float[] wt, int _wt_offset, float theta, int col,
            int head, float[] p, int _p_offset, float[] c, int _c_offset,
            float[] wbp, int _wbp_offset, float[] v, int _v_offset, ref int nseg,
            int iprint, float sbgnrm, ref int info, float epsmch)
        {
            ...
            // 
            {
                for (i = 1; i <= n; i++)
                {
                        ...
                        && (iwhere[(i - (1)) + _iwhere_offset] != -1)))
                    {
                        ...
                        xlower = ((nbd[(i - (1)) + _nbd_offset] <= 2) && (tl <= 0));
                        xupper = ((nbd[(i - (1)) + _nbd_offset] >= 2) && (tu <= 0));
                        // 
                        // c              reset iwhere(i).
                        iwhere[(i - (1)) + _iwhere_offset] = 0;
                        if (xlower)
                    ...
                    else
                    {
                        ...
                        else if (((nbd[(i - (1)) + _nbd_offset] >= 2) && (neggi > 0)))
                        {
                            // x(i) + d(i) is bounded; compute t(i).
            ...
            if ((col > 0))
            {
                ...
                f2 = (f2 - BFGS_F.ddot(col2, v, _v_offset, 1, p, _p_offset, 1));
            }
        ...
        // 
        // 
        private static void cmprlb(int n, int m, float[] x, int _x_offset, float[] g, int _g_offset, float[] ws, int _ws_offset, float[] wy, int _wy_offset, float[] sy, int _sy_offset,
        float[] wt, int _wt_offset, float[] z, int _z_offset, float[] r, int _r_offset,
        float[] wa, int _wa_offset, int[] index, int _index_offset, float theta,
        ...
        // c     where
        // c
        // c       f is a float precision variable.
        ...
        // c
        // c       g is a float precision variable.
        ...
        // c
        // c       stp is a float precision variable. 
        ...
        // c         
        // c       dsave is a float precision work array of dimension 13.
        // c
        // c     Subprograms called
        // c
        // c       minPACK-2 ... dcstep
        ...
        // 
        private static void dcsrch(float f, float g, ref float stp, float ftol,
        float gtol, float xtol, float stpmin, float stpmax, ref Task task,
        int[] isave, int _isave_offset, float[] dsave, int _dsave_offset)
        {
            bool brackt = false;
            int stage = 0;
            float finit = 0f;
            ...
            {
                stmin = (stp + (1.100000000000000088817841970012523233891f * ((stp - stx))));
        ...
        // c     where
        // c
        // c       stx is a float precision variable.
        ...
        // c
        // c       sty is a float precision variable.
        ...
        // c
        // c       fp is a float precision variable.
        // c         On entry fp is the function at stp
        // c         On exit fp is unchanged.
        // c
        // c       dp is a float precision variable.
        ...
        // 
        // 
        internal static void dcstep(ref float stx, ref float fx, ref float dx,
            ref float sty, ref float fy, ref float dy, ref float stp,
            float fp, float dp, ref bool brackt, float stpmin, float stpmax)
        {
            float gamma = 0f;
            float p = 0f;
            float q = 0f;
            ...
            if ((fp > fx))
            {
                theta = ((((3.0f * ((fx - fp))) / ((stp - stx))) + dx) + dp);
                ...
                r = (p / q);
                stpc = (stx + (r * ((stp - stx))));
                stpq = (stx + (((((dx / (((((fx - fp)) / ((stp - stx))) + dx)))) / 2.0f)) * ((stp - stx))));
            ...
            else if ((math.abs(dp) < math.abs(dx)))
            {
                ...
                if (brackt)
                {
                    ...
                    {
                        stpf = math.min((stp +
                            (0.6600000000000000310862446895043831318617f * ((sty - stp)))), stpf);
                    }
                    else
                    {
                        stpf = math.max((stp +
            ...
            else
            {
                if (brackt)
                {
                    theta = ((((3.0f * ((fp - fy))) / ((sty - stp))) + dy) + dp);
        ...
        // 
        private static void errclb(int n, int m, float factr,
            float[] l, int _l_offset, float[] u, int _u_offset,
        ...
        // c       On entry iupdat is the total number of BFGS_F updates made so far.
        // c       On exit iupdat is unchanged. 
        // c
        // c     updatd is a logical variable.
        // c       On entry 'updatd' is true if the L-BFGS_F matrix is updatd.
        // c       On exit 'updatd' is unchanged. 
        // c
        // c     wn is a float precision array of dimension 2m x 2m.
        ...
        // c                     [L_a -R_z           theta*S'AA'S ]
        // c
        // c     wn1 is a float precision array of dimension 2m x 2m.
        ...
        // c       On exit m is unchanged.
        // c
        // c     ws, wy, sy, and wtyy are float precision arrays;
        // c     theta is a float precision variable;
        // c     col is an integer variable;
        // c     head is an integer variable.
        ...
        // c       SIAM J. Scientific Computing 16 (1995), no. 5, pp. 1190--1208.
        // c
        // c       [2] C. Zhu, R.H. Byrd, P. Lu, J. Nocedal, ``L-BFGS_F-B: a
        ...
        private static void formk(int n, int nsub, int[] ind, int _ind_offset,
            ...
            int iupdat, bool updatd, float[] wn, int _wn_offset, float[] wn1, int _wn1_offset,
            int m, float[] ws, int _ws_offset, float[] wy, int _wy_offset,
            float[] sy, int _sy_offset, float theta, int col, int head,
            ref int info)
        {
        ...
        private static void formt(int m,
            float[] wt, int _wt_offset,
            float[] sy, int _sy_offset,
            float[] ss, int _ss_offset,
            int col, float theta, ref int info)
        {
        ...
        // c       On exit n is unchanged.
        // c
        // c     t is a float precision array of dimension n.
        ...
        // 
        // 
        private static void hpsolb(int n, float[] t, int _t_offset,
        ...
        // 
        // 
        private static void lnsrlb(int n, float[] l, int _l_offset, float[] u, int _u_offset,
            int[] nbd, int _nbd_offset, float[] x, int _x_offset, float f, ref float fold, ref float gd,
            ref float gdold, float[] g, int _g_offset, float[] d, int _d_offset, float[] r, int _r_offset,
            float[] t, int _t_offset, float[] z, int _z_offset, ref float stp,
            ref float dnorm, ref float dtd, ref float xstep, ref float stpmx, int iter,
            ...
            float[] dsave, int _dsave_offset)
        {
                ...
                {
                    {
                        for (i = 1; i <= n; i++)
                        {
                            ...
                            if ((nbd[(i - (1)) + _nbd_offset] != 0))
                            {
                                ...
                                else if (((a1 > 0) && (nbd[(i - (1)) + _nbd_offset] >= 2)))
                                {
            ...
            if (((iter == 0) && (!boxed)))
            {
                if (float.IsNaN(dnorm))
                    stp = stpmx;
                else
                    stp = math.min((1.0f / dnorm), stpmx);
            }
            else
            {
                stp = 1.0f;
        ...
        L556:
            gd = BFGS_F.ddot(n, g, _g_offset, 1, d, _d_offset, 1);
            ...
            dcsrch(f, gd, ref stp,
                0.01000000000000000020816681711721685132943f,
                0.9000000000000000222044604925031308084726f,
                0.1000000000000000055511151231257827021182f, 0,
        ...
        // c       using the compact formula of the limited memory BFGS_F updates.
        // c       
        // c     n is an integer variable.
        ...
        // c
        // c     factr is a float precision variable.
        ...
        // c
        // c     pgtol is a float precision variable.
        ...
        // c
        // c     ws, wy, sy, and wt are float precision working arrays used to
        ...
        // c                                  (2.26) in [3].
        // c
        // c     wn is a float precision working array of dimension 2m x 2m
        ...
        // c                     [ 0  I]
        // c
        // c     snd is a float precision working array of dimension 2m x 2m
        // c       used to store the lower triangular part of
        // c                 N = [Y' ZZ'Y   L_a'+R_z']
        // c                     [L_a +R_z  S'AA'S   ]
        // c            
        // c     z(n),r(n),d(n),t(n), xp(n),wa(8*m) are float precision working ar
        // c       z  is used at different times to store the Cauchy point and
        // c          the Newton point.
        // c       xp is used to safeguard the projected Newton direction
        // c
        // c     sg(m),sgo(m),yg(m),ygo(m) are float precision working arrays. 
        // c
        ...
        // c
        // c     dsave is a float precision working array of dimension 29.
        // c
        // c
        // c     Subprograms called
        // c
        // c       L-BFGS_F-B Library ... cauchy, subsm, lnsrlb, formk, 
        // c
        ...
        private static void mainlb(int n,
            ...
            float[] ss, int _ss_offset,
            float[] wt, int _wt_offset,
            float[] wn, int _wn_offset,
            float[] snd, int _snd_offset,
            float[] z, int _z_offset,
            float[] r, int _r_offset,
            float[] d, int _d_offset,
            float[] t, int _t_offset,
            float[] xp, int _xp_offset,
            ...
            float[] dsave, int _dsave_offset)
        {
            ...
            {
                // 
                // epsmch = (float)(Epsilon.epsilon(1.0f));
                epsmch = math.EPSILON;
                ...
                // 
                // c           for the limited memory BFGS_F matrices:
                col = 0;
                head = 1;
                theta = 1.0f;
                iupdat = 0;
                ...
                if (task == Task.Error)
                {
                    ...
                    Prn3lb.prn3lb(n, x, _x_offset, f, task, iprint,
                        info, itfile, iter, nfgv, nintol, nskip, nact, sbgnrm, 0, nseg, word,
        ...
        L777:
            ...
            ddum = math.max(math.abs(fold), math.max(math.abs(f), 1.0f));
            ...
            if ((dr <= (epsmch * ddum)))
            {
                ...
                {
                    // DISPLAY: dr, ddum
                    // DISPLAY: '  ys=',1p,e10.3,'  -gs=',1P,E10.3,' BFGS_F update SKIPPED'"
                }
        ...
        internal static void matupd(int n,
            int m, float[] ws, int _ws_offset, float[] wy, int _wy_offset,
            float[] sy, int _sy_offset, float[] ss, int _ss_offset, float[] d, int _d_offset,
            float[] r, int _r_offset, ref int itail, int iupdat, ref int col,
            ref int head, ref float theta, float rr, float dr, float stp, float dtd)
        {
        ...
        // c                                                                       
        // c  L-BFGS_F-B is released under the �New BSD License� (aka �Modified
        // c  or �3-clause license�)                                           
        // c  Please read attached file License.txt                                
        // c                                        
        // c===========   L-BFGS_F-B (version 3.0.  April 25, 2011  =================
        // c
        // c     This is a modified version of L-BFGS_F-B. minor changes in the updat
        ...
        // c         L-BFGS_F-B: Fortran Subroutines for Large-Scale Bound Constraine
        ...
        // c       then uses the limited memory BFGS_F method to solve the bound
        ...
        // c       On exit pgtol is unchanged.
        // c
        // c     wa is a float precision working array of length 
        ...
        // c         isave(26) = the total number of skipped BFGS_F updates before 
        // c                         the current iteration;
        // c         isave(30) = the number of current iteration;
        // c         isave(31) = the total number of BFGS_F updates prior the current
        // c                         iteration;
        ...
        // c       On exit with 'task' = NEW_X, the following information is
        // c                                                             available:
        // c         dsave(1) = current 'theta' in the BFGS_F matrix;
        ...
        // c      "Remark On Algorithm 788: L-BFGS_F-B: Fortran Subroutines for Large
        ...
        // c       the active set at xcp, and an l-BFGS_F matrix B 
        ...
        // c          d = -(Z'BZ)^(-1) r.
        // c
        // c       The formula for the Newton direction, given the L-BFGS_F matrix
        ...
        // c
        // c     wv is a float precision working array of dimension 2m.
        // c
        // c     wn is a float precision array of dimension 2m x 2m.
        ...
        float[] l, int _l_offset, float[] u, int _u_offset,
        int[] nbd, int _nbd_offset, float[] x, int _x_offset,
        float[] d, int _d_offset, float[] xp, int _xp_offset,
        float[] ws, int _ws_offset, float[] wy, int _wy_offset,
        float theta, float[] xx, int _xx_offset, float[] gg, int _gg_offset,
        int col, int head, ref int iword,
        float[] wv, int _wv_offset, float[] wn, int _wn_offset, int iprint, ref int info)
        {
            int pointr = 0;
            int m2 = 0;
            int col2 = 0;
            ...
            dscal(nsub, (1.0f / theta), d, _d_offset, 1);
            ...
            ibd = 0;
            {
                for (i = 1; i <= nsub; i++)
                {
                    ...
                    if ((nbd[(k - (1)) + _nbd_offset] != 0))
                    {
                        ...
                        else if (((dk > 0) && (nbd[(k - (1)) + _nbd_offset] >= 2)))
                        {
        ...
        // c                                        
        // c
        // c     dpofa factors a float precision symmetric positive definite
        // c     matrix.
        // c
        ...
        // c
        // c
        private static void dpofa(float[] a, int _a_offset, int lda, int n, ref int info)
        {
            float t = 0f;
            float s = 0f;
            int j = 0;
        ...
        // *     .. Intrinsic Functions ..
        // *     ..
        private static void dscal(int n, float da, float[] dx, int _dx_offset, int incx)
        {
        ...
        // *     .. Intrinsic Functions ..
        // *     ..
        private static float ddot(int n, float[] dx, int _dx_offset,
           int incx, float[] dy, int _dy_offset, int incy)
        {
            float dtemp = 0f;
            int i = 0;
            int ix = 0;
        ...
        private static void dcopy(int n, float[] dx, int _dx_offset, int incx,
           float[] dy, int _dy_offset, int incy)
        {
        ...
        // *     .. Intrinsic Functions ..
        // *     ..
        private static void daxpy(int n, float da,
             float[] dx, int _dx_offset, int incx, float[] dy, int _dy_offset, int incy)
        {
