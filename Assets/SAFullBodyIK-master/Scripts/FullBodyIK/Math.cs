...
namespace SA
{
	public partial class FullBodyIK
	{
        ...
        //这个需要花点时间
        //OYM：大概看明白了，这个作者做了一个....不应该优化的优化，这样让可读性反而变得很差了
        //OYM：首先， 因为旋转矩阵是一个正交矩阵，所以Rot-1=Rot T
        //OYM：这里直接省略了Rot T的步骤，默认rhs为需要旋转的矩阵，所以是 A*B-1一样的操作，有因为B-1=B t，所以是行*行这种格式..
        //OYM：真没必要，这里省下来的时间还不如优化一下存储什么的
		...
		{
            return SAFBIKComputeBasisFromXYLockY(out basis, ref dirX, ref dirY);//OYM：多套了一层ref
        }
