...
namespace SA
{
	public partial class FullBodyIK
	{
		...
		public class Bone
		{
			...
			int _fingerIndex = -1;
			[SerializeField]
            _LocalAxisFrom _localAxisFrom = _LocalAxisFrom.Unknown; //OYM：这个暂时不清楚
            [SerializeField]
            _DirectionAs _localDirectionAs = _DirectionAs.Uknown; //OYM：目标骨骼的基础向量方向什么的（话说为啥没有z轴
            ...
            void _PresetLocalAxis(_LocalAxisFrom localAxisFrom, _DirectionAs localDirectionAs)//OYM：确认Axis与DirectionAs
            {
			...
			public void Prepare( FullBodyIK fullBodyIK )
			{
				...
				if( this.transformIsAlive ) {
                    ...
                    SAFBIKMatSetRot(out _defaultBasis, ref _defaultRotation);//OYM：旋转转换成旋转矩阵
                    if( _parentBone != null ) { // Always _parentBone.transformIsAlive == true
                        _defaultLocalTranslate = _defaultPosition - _parentBone._defaultPosition;//OYM：初始的相对位置（包含了localRotation
                        _defaultLocalLength = FastLength.FromVector3(ref _defaultLocalTranslate);//OYM：同时存储length与lengthsq，可怜娃不知道读取比乘法至少慢了一个数量级
                        if (_defaultLocalLength.length > FLOAT_EPSILON)//OYM：这个E应该是zreo的
                        { //OYM：
                    ...
                    SAFBIKMatMultInv0(out _worldToBaseBasis, ref _defaultBasis, ref fullBodyIK.internalValues.defaultRootBasis);//OYM：这里应该是root to base 吧。。。。
                    _baseToWorldBasis = _worldToBaseBasis.transpose;
                    SAFBIKMatGetRot(out _worldToBaseRotation, ref _worldToBaseBasis);	//OYM：矩阵转四元数，其实直接求四元数的逆会快一点
