...
namespace SA
{
	...
	public partial class FullBodyIK
	{
		...
		{
            public Bone shoulder;//OYM：肩
            public Bone arm;//OYM：手臂
            public Bone[] armRoll;//OYM： 不知道
            public Bone elbow;//OYM：肘
            public Bone[] elbowRoll;//OYM：不知道
            public Bone wrist;//OYM：手腕

            public void Repair()
			{
                SafeResize(ref armRoll, MaxArmRollLength);//OYM：创建
		...
		public class Settings
		{
			...
			[System.Serializable]
            public class BodyIK //OYM：一堆参数，回头再来看
            {
        ...
        public void Awake(Transform rootTransorm_)//OYM：在这里初始化
        {
        ...
        void _Prefix()//OYM：准备所有的数据，因为骨骼内部分很多种，而描述骨骼的类又只有一个，所以就用了这个泛化的手段去描述
        {
			if( _isPrefixed ) {
				return;
			}
            ...
            SafeNew(ref bodyBones);//OYM：这个有点意思，是一个安全加载的方法，可以快速检测该物体有没有被示例化
            //OYM：但是我觉得，懒加载是个更好的决定
            //OYM：无所谓啦
            ...
            SafeNew(ref leftArmBones);//OYM：这里其实都是Bone的集合体，我估摸着后续会传入内部
            leftArmBones.Repair();
            ...
            leftHandFingersBones.Repair();//OYM：创建数组（new不好吗
		...
		public void ConfigureBoneTransforms()
		{
            ...
            {//OYM：是否自动转换humanoid
				...
				if( animator != null && animator.isHuman ) {
                    ...
                    //OYM：如果没有脖子，但是有脑袋的话，就去自动找一下脖子，实在找不到就与head共用
                    if( neck == null ) {
						if( head != null ) {
					...
					if( leftEye == null ) {
                        leftEye = _FindEye(head, false);//OYM：根据名字寻找眼睛
                    }
					if( rightEye == null ) {
		...
		{
			Assert( _bones != null );
            Bone.Prefix(_bones, ref bone, boneLocation, parentBoneLocationBased);//OYM：映射骨骼axis与骨骼的direction
        }
